...
In case you cannot get the auto install script (highly unlikely), the contents in it was recorded:
...
egrep ^menuentry /etc/grub2.cfg | cut -f 2 -d \' # 查看所有可引导的内核列表
grub2-set-default 0 # 选择 0 即启用显示的第一个内核
reboot
echo "net.core.default_qdisc=fq" >> /etc/sysctl.conf
echo "net.ipv4.tcp_congestion_control=bbr" >> /etc/sysctl.conf
sysctl -p
sysctl -n net.ipv4.tcp_congestion_control
lsmod | grep bbr # 验证是否开启
```
